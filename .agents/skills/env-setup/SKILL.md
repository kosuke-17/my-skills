---
name: env-setup
description: プロジェクトの環境構築を行うスキル。「環境構築して」「プロジェクトをセットアップして」「新しいプロジェクトを作りたい」「開発環境を整えて」「init して」などのリクエスト時に使用する。技術スタックは決めうちで、references/stack.md に定義されている。
---
...
2-1. pnpm を有効化する（stack.md の `パッケージマネージャー.setup` を実行）
